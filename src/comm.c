...
void close_socket( DESCRIPTOR_DATA *dclose )
{
	...
	{
	    act( "$n has lost consciousness.", ch, NULL, NULL, TO_ROOM );
	 ...
	 case 's' :
	    if( ch->in_room != NULL )
	       sprintf( buf2, "%s", 
	        ...
	        ? sector_flags[ch->in_room->sector_type].name : "unknown");
	    else
	       sprintf( buf2, " ");
	    i = buf2; break;
         case 'a' :
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
	...
	sprintf( log_buf, "%s@%s has regained consciousness.", ch->name, d->host );
	log_string( log_buf );
...
/*	for ( race = 1; race_table[race].name != NULL; race++ )
	...
	if (!strcmp(arg,"help"))
	{
		...
		do_function(ch, &do_help, argument); /*argument);*/
            write_to_buffer(d,
...
case CON_GET_ALIGNMENT:
	switch( argument[0])
	{
		...
		write_to_buffer(d,"Which alignment (Good/Neutral/Evil)? ",0);
		return;
	}
...
case CON_DEFAULT_CHOICE:
	...
	write_to_buffer( d, 
    "\n\rReality gives way to the world known as The Desolate Planes.\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
	d->connected	= CON_PLAYING;
	...
	act( "$n has entered the world.", ch, NULL, NULL, TO_ROOM );
	...
	if (ch->pet != NULL)
	{
	    ...
	    act("$n has entered the world.",ch->pet,NULL,NULL,TO_ROOM);
	}
...
bool check_reconnect( DESCRIPTOR_DATA *d, char *name, bool fConn )
{
	...
	&&   !str_cmp( d->character->name, ch->name ) )
	{
		...
		sprintf( log_buf, "%s@%s regained consciousness.", ch->name, d->host );
		log_string( log_buf );
...
void stop_idling( CHAR_DATA *ch )
{
    ...
    ch->was_in_room	= NULL;
    act( "$n has returned from the abyss.", ch, NULL, NULL, TO_ROOM );
    return;
}
 ...
 */
/*void page_to_char( const char *txt, CHAR_DATA *ch ) geadin commented */
	      ...
	      const void *arg2, int type, int min_pos)
{
    ...
    {
        /*if( !to->desc || to->position < min_pos )  ### by Geadin */
