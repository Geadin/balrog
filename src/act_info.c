...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    if ( IS_AFFECTED(ch, AFF_DETECT_EVIL)
         && IS_OBJ_STAT(obj, ITEM_EVIL)   )   strcat( buf, "(Black Aura) "  );
    if (IS_AFFECTED(ch, AFF_DETECT_GOOD)
    &&  IS_OBJ_STAT(obj,ITEM_BLESS))	      strcat(buf,"(Golden Aura) "	);
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    {
    case POS_DEAD:     strcat( buf, " is SLAIN!!" );              break;
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
	...
	strcat(buf, " is mangled and bleeding to death on the ground.\n\r");
...
void do_scroll(CHAR_DATA *ch, char *argument)
{
    ...
    {
	send_to_char("You must provide a number inbetween 1 and 100.\n\r",ch);
	return;
    }
...
void do_autolist(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(ch->act,PLR_AUTOLOOT))
        send_to_char("You are automatically looting.\n\r",ch);
    else
        send_to_char("You aren't looting corpses.\n\r",ch);

    send_to_char("autosac        ",ch);
    if (IS_SET(ch->act,PLR_AUTOSAC))
        send_to_char("You are sacrificing corpses.\n\r",ch);
    else
        send_to_char("You are no longer sacrificing corpses.\n\r",ch);

    send_to_char("autosplit      ",ch);
    if (IS_SET(ch->act,PLR_AUTOSPLIT))
        send_to_char("You are splitting gold.\n\r",ch);
    else
        send_to_char("You only share the coins with yourself.\n\r",ch);
	...
	send_to_char("You do not allow followers.\n\r",ch);
    else
...
void do_autogold(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(ch->act,PLR_AUTOGOLD))
    {
      send_to_char("You no longer loot gold.\n\r",ch);
      REMOVE_BIT(ch->act,PLR_AUTOGOLD);
    }
    else
    {
      send_to_char("You automatically loot gold.\n\r",ch);
...
void do_autoloot(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(ch->act,PLR_AUTOLOOT))
    {
      send_to_char("You are no longer looting corpses.\n\r",ch);
      REMOVE_BIT(ch->act,PLR_AUTOLOOT);
    }
    else
    {
      send_to_char("You automatically loot corpses.\n\r",ch);
...
void do_autosac(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(ch->act,PLR_AUTOSAC))
    {
      send_to_char("You no longer sacrifice corpses.\n\r",ch);
      REMOVE_BIT(ch->act,PLR_AUTOSAC);
    }
    else
    {
      send_to_char("You are now sacrificing corpses.\n\r",ch);
...
void do_autosplit(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(ch->act,PLR_AUTOSPLIT))
    {
      send_to_char("You no longer split gold.\n\r",ch);
      REMOVE_BIT(ch->act,PLR_AUTOSPLIT);
    }
    else
    {
      send_to_char("You are now splitting the coin.\n\r",ch);
...
void do_brief(CHAR_DATA *ch, char *argument)
{
    ...
    {
      send_to_char("You now see full room descriptions.\n\r",ch);
      REMOVE_BIT(ch->comm,COMM_BRIEF);
    }
    else
    {
      send_to_char("Room descriptions removed.\n\r",ch);
...
void do_prompt(CHAR_DATA *ch, char *argument)
{
   ...
   if ( argument[0] == '\0' )
   {
	if (IS_SET(ch->comm,COMM_PROMPT))
   	{
      	    send_to_char("You will no longer your prompts.\n\r",ch);
      	    REMOVE_BIT(ch->comm,COMM_PROMPT);
    	}
    	else
    	{
      	    send_to_char("You will now your prompts.(HELP PROMPT)\n\r",ch);
