...
{
    1, 2, 2, 3, 4, 6, 4, 1, 6, 10, 6, 4, 4, 3, 4, 5, 1, 5, 4
};
...
void move_char( CHAR_DATA *ch, int door, bool follow )
{
    ...
    {
	send_to_char( "What?  And leave your master behind?\n\r", ch );
	return;
    }
	...
	for ( iClass = 0; iClass < MAX_CLASS; iClass++ )
	{
	    for ( iGuild = 0; iGuild < MAX_GUILD; iGuild ++)	
	    {
	    	...
	    	{
		    send_to_char( "You aren't a member of this guild.\n\r", ch );
		    return;
		}
	    }
	}
  	...
  	&&    !IS_AFFECTED(ch,AFF_FLYING))
	{
	    ...
	    if ( !found )
	    {
		send_to_char( "You will need a boat to go there.\n\r", ch );
		return;
	    }
	}
	...
	if ( ch->move < move )
	{
	    send_to_char( "Your legs are too tired.\n\r", ch );
	    return;
	}
	...
	act( "$n walks $T.", ch, NULL, dir_name[door], TO_ROOM );

    char_from_room( ch );
    char_to_room( ch, to_room );
    if ( !IS_AFFECTED(ch, AFF_SNEAK)
    &&   ch->invis_level < LEVEL_HERO)
	act( "$n walks in.", ch, NULL, NULL, TO_ROOM );
...
void do_close( CHAR_DATA *ch, char *argument )
{
	...
	if (obj->item_type == ITEM_PORTAL)
	{
	    ...
	    {
		send_to_char("That can't be closed.\n\r",ch);
		return;
	    }
	...
	send_to_char( "You close the door.\n\r", ch );
...
void do_pick( CHAR_DATA *ch, char *argument )
{
	...
	{
	    act( "$N is standing in front of the lock.",
		ch, NULL, gch, TO_CHAR );
	    return;
	}
    }
	...
	if (obj->item_type == ITEM_PORTAL)
	{
	    ...
	    {
		send_to_char("This lock can't be picked.\n\r",ch);
		return;
	    }
	...
	REMOVE_BIT(pexit->exit_info, EX_LOCKED);
	send_to_char( "You successfully pick the lock.\n\r", ch );
...
void do_visible( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "You become visible to everyone.\n\r", ch );
    return;
}
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You will need to find other means for recalling.\n\r",ch);
	return;
    }
