...
map[x][y]=room->sector_type;
    ...
    if ( IS_NPC(ch)   //comment for geadin
...
case SECT_FIELD:	send_to_char("{Go",ch);	break;
case SECT_HILLS:	send_to_char("{G^",ch);		break;
//case SECT_INSIDE:		send_to_char("{m+",ch);		break;
...
/* mlk prioritizes*/
if (map[x][y-1]==SECT_INSIDE) map[x][y]=SECT_INSIDE;
if (map[x][y-1]==SECT_CITY) map[x][y]=SECT_CITY;
...
{
  if (  !IS_SET(ch->in_room->room_flags, SECT_FOREST)   )
...
case SECT_INSIDE:		sector_name="a road";break;
...
strcpy(desc,"You are in a forest teeming with life.  All around are the sure signs of nature.");
  ...
  if
(((exit[0]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[0]==SECT_INSIDE)||(exit[0]==SECT_RUINS)){
    sprintf(buf,"You see %s to the north.",get_sector_name(exit[0]));
    strcat(desc,buf);strcpy(buf,"  ");
  }
  if
(((exit[1]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[1]==SECT_INSIDE)||(exit[1]==SECT_RUINS)){
    sprintf(buf,"To the east you can see %s.",get_sector_name(exit[1]));
    strcat(desc,buf);strcpy(buf,"  ");
  }
  if (((exit[2]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[2]==SECT_INSIDE)||(exit[2]==SECT_RUINS)){
    sprintf(buf,"South of you, you can see %s.",get_sector_name(exit[2]));
    strcat(desc,buf);strcpy(buf,"  ");
  }
  if (((exit[3]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[3]==SECT_INSIDE)||(exit[3]==SECT_RUINS)){
...
Insects fly through the air in occasional swarms.");
  ...
  if (((exit[0]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[0]==SECT_INSIDE)||(exit[0]==SECT_RUINS)){
    sprintf(buf,"You see %s to the north.",get_sector_name(exit[0]));
    strcat(desc,buf);strcpy(buf,"  ");
  }
  if (((exit[1]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[1]==SECT_INSIDE)||(exit[1]==SECT_RUINS)){
    sprintf(buf,"To the east you can see %s.",get_sector_name(exit[1]));
    strcat(desc,buf);strcpy(buf,"  ");
  }
  if (((exit[2]!=(room->sector_type))&&(number_percent()<CHANCE))||(exit[2]==SECT_INSIDE)||(exit[2]==SECT_RUINS)){
...
");
  ...
  if
(((exit[0]!=(room->sector_type))&&(number_percent()<CHANCE+20))||(exit[0]==SECT_INSIDE)||(exit[0]==SECT_RUINS)){
    sprintf(buf,"You see %s to the north.",get_sector_name(exit[0]));
    strcat(desc,buf);strcpy(buf,"  ");
  }

if(((exit[1]!=(room->sector_type))&&(number_percent()<CHANCE+20))||(exit[1]==SECT_INSIDE)||(exit[1]==SECT_RUINS)){
    sprintf(buf,"To the east you can see %s.",get_sector_name(exit[1]));
    strcat(desc,buf);strcpy(buf,"  ");
  }

if(((exit[2]!=(room->sector_type))&&(number_percent()<CHANCE+20))||(exit[2]==SECT_INSIDE)||(exit[2]==SECT_RUINS)){
    sprintf(buf,"South of you, you can see %s.",get_sector_name(exit[2]));
    strcat(desc,buf);strcpy(buf,"  ");
  }

if(((exit[3]!=(room->sector_type))&&(number_percent()<CHANCE+20))||(exit[3]==SECT_INSIDE)||(exit[3]==SECT_RUINS)){
    sprintf(buf,"%s can be seen to the west.",get_sector_name(exit[3]));
    strcat(desc,buf);strcpy(buf,"  ");
  }
break;
...
lk this sets the whole wilderness default descs and names
